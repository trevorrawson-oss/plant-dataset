...
def calendar_presence_violations(crop):
    ...
    blank planner. No-op off frost_anchored: tree empty cells are governed by A3's no-fruit split
    (unsuitable / chill-limited cells are legitimately empty); indoor crops have no cells."""
    ...
    for rk, r in (crop.get("regions") or {}).items():
        ...
        for z, cell in (r.get("resolved_by_zone") or {}).items():
            ...
            if not (cell.get("calendar") or []):
                V.append(f"{rk}.{z}: frost_anchored resolved cell has an empty/absent calendar "
                         f"-- a frost_anchored cell must render a non-empty month-strip calendar")
    return V

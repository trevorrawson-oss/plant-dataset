#!/usr/bin/env python3
"""Merge nevada authoring shard files into a class staging file, then report cell count +
gate readiness. Controller-only build helper (subagents never commit).

Usage:
  python3 tools/staging/nevada_merge.py <class_staging_file> <shard1.json> [shard2.json ...]
    ...
    for sp in shards:
        ...
        for slug, cell in s.items():
            ...
            if slug not in data:
                added.append(slug)
            data[slug] = cell
    # HERE = tools/, so target/shard paths are given relative to repo root by the caller

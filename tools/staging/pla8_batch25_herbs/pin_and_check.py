#!/usr/bin/env python3
"""pin_and_check -- PLA-8 batch 25 (herbs): build the TARGET-STATE scratch and run the PLA-449
...
WHY THE SPEC IS A TARGET STATE, NOT AN ID MAP. This batch renames three entries, splits four bundles
into nine, deletes one bundle half and retires two entries. The names the guard must see are the
names that will SHIP, not canonical's. An id map over canonical would hand the guard the old names
and report a clean that means nothing.
...
EXPECTED_BASE_SHA = "500a61262d5870636d8b33845cb81072940e677d3674938c0375319eab6d6fc9"  # a9c84847 + the uc_ipm_pn7493 catalog admission
...
def run_gate(path, minted, strict=True):
    ...
    if strict:
        cmd.append("--strict")
    print("\n$ problem_id_collision_gate.py <scratch> --minted <21 ids> --strict\n" + "=" * 78)
...
def main():
    ...
    if args.self_test:
        ...
        pidx["thyme"]["pests"][0]["id"] = "spider-mite"   # roster has `spider-mites`, distance 1
        probe_path = SCRATCH + ".selftest"
        with open(probe_path, "w", encoding="utf-8") as f:
            json.dump(probe, f, ensure_ascii=False)
        rc, out = run_gate(probe_path, minted | {"spider-mite"})
        os.unlink(probe_path)
        if "spider-mite" not in out or rc == 0:
            sys.exit("HARNESS DEAD: planted collision `spider-mite` did not redden the guard")
    ...
    # not a failure to fix: batch 25 JOINS ids that already carry known duplicate pairs (mint joins
    # `cutworms`, which has collided with asparagus's `cutworm` since long before this batch; sage
    # joins `slugs-and-snails`, one of the three-id slug family). Those are PLA-448 s4a's eight
    # duplicates, whose merge is s7's fast-follow and is explicitly out of PLA-8's scope.
    #
    ...
    # by registration is exactly how a registration path decays into a blind spot.
    #

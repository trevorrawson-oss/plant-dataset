...
**Owed with it, not in it:** ~~the A59 N/A predicate~~ **DONE 2026-09-18, see below**; the mandarin satsuma-anchor repair; the Plan E note that cherry-sweet's and both pears' rootstock-row heights are not T1-stated in feet (noted on PLA-463).

## A59 coverage rule (2026-09-18, tooling only, canonical unchanged)

A certified woody crop may carry a null `mature_height_ft` only if it is cane fruit or a `verification_status` record NAMES the field. Armed at birth: green on the canonical carrying the rulings (16 authored, 2 cane-exempt, 10 recorded, 0 unexplained across 28 woody crops). This makes the promote-2 rulings load-bearing rather than documentary.

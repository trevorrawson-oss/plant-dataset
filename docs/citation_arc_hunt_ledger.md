...
**Last reconciled:** 2026-08-05, canonical `ca40d90f` (pushed, `origin/main` at `0c276ec`). Campaign A is live; campaign B closed
2026-08-04; **campaign C CLOSED 2026-08-05 (PLA-113)** in TWO promotes
(`5a52a76c` -> `754c51a0` -> `ca40d90f`) -- see the campaign table,
`docs/2026-08-05-campaign-c-reprice-and-arid-document-read.md` and
`docs/2026-08-05-az1005-and-divergence-followups.md`. **`campaign_c_reprice` reports 0 open of 25.**
...
| 7 | `warm_arid` | `nmsu_ext` | 9 | **CLOSED-RULING** | **CLOSED 2026-08-05** (campaign C). All 9 decisions carry a crop-scoped pilot finding naming `nmsu_ext` as an institution portal; pumpkin's was filed by the closeout, the only crop in its sibling set of six lacking one. Cells stay bare BY DESIGN, so the scan keeps counting them. Standing caution unchanged: `right-document-wrong-claim` -- NMSU CR457B has no planting-date window at all. |
| 8 | `warm_arid` | `tamu_agrilife` | 9 | **PARTIAL** | **CLOSED 2026-08-05 for all 8 vegetables** (declared by their own `_regional_source_anchors_general` findings, pumpkin's filed by the closeout). Residue: `lemon` -> campaign D. |
...
| 13 | `rgv` | `tamu_agrilife` | 6 | **CLOSED-RULING** | **CLOSED 2026-08-05.** The RGV Homeowner Vegetable Guide was cached as an HTTP 403 both scans read as ABSENCE; refetched, it has **ZERO rows** for all six crops and its only pea is **Cowpea** (*Vigna*, wrong genus). All six already declared that absence in their own `synthesis_note_seasoned`; the closeout moved it into the findings ledger, one finding per crop. garlic split by arm: `plant_out` repointed to TAMU Bexar's garlic page, harvest arms held with the Apr 13 divergence filed. |
| 14 | `low_desert_az` | `uariz_ext` | 6 | **PARTIAL** | **CLOSED 2026-08-05 for all 4 vegetables.** okra declared by `okra_pilot_region_anchor_base_urls`; cantaloupe, honeydew-melon and watermelon REPOINTED to `uariz_ext_az1005` after AZ1005 was read from character coordinates (its grid IS 90°-rotated, confirmed mechanically, and its text layer carries no dates at all). watermelon's 2 `second_planting` nodes held bare: AZ1005 gives it a spring window ONLY. Residue: `lemon`, `lime` -> campaign D. |
...
| 17 | `warm_arid` | `nmsu_donaana_mg` | 2 | **CLOSED-EDIT** | **CLOSED 2026-08-05.** Both tomatoes' zone 8 cells named their document in their own `basis_seasoned` prose -- the Dona Ana Las Cruces planting chart -- and were repointed at it. That chart's windows are GRAPHICAL BARS with no dates in the text layer, so it was read visually; its tomato transplant rows (Mar into early Apr, mid-Jul into early Aug) MATCH the cells. |
...
| 21 | `ca_desert` | `uariz_ext` | 2 | **DEFERRED** | 100% citrus (lemon + lime, nothing else) -> campaign D, per kickoff 53 s4a. Moved wholesale rather than read twice against two ledgers. |
| 22 | `<crop-level>` | `ucr_citrus` | 2 | **CLOSED-RULING** | As #15. |
| 23 | `se_gulf` | `uga_ext` | 1 | OPEN | |
| 24 | `warm_arid` | `nmsu_chart` | 1 | **CLOSED-EDIT** | **CLOSED 2026-08-05.** NOT the mis-classification this ledger guessed: carrot's zone 8 cell and its `heat_pause` cited `nmsu_chart` (New Mexico State) at `https://desert.tamu.edu/`, a bare **TEXAS** host, while the catalog held the right document all along. `nmsu_chart` carried three urls across 15 nodes and 5 crops. Both nodes repointed; the claim is corroborated by the chart's carrot row. Surfaced the `catalog_divergence_scan` class. |
...
| **C. Arid + Texas** | 7 (#7, #8, #13, #14, #17, #21, #24) | **35 -> 6 -> 0 OPEN** | NMSU + TAMU AgriLife + Arizona across `warm_arid`/`rgv`/`low_desert_az`/`ca_desert` | **CLOSED 2026-08-05 (PLA-113)** in ONE promote (`5a52a76c` -> `754c51a0`): **5 nodes repointed, 29 held bare with per-crop stated reasons, 8 findings filed, no catalog id minted**, guards proving no consumer string moved and 40 standalone gates byte-identical. **FOLLOW-UPS ALSO CLOSED the same day** in a second promote (`754c51a0` -> `ca40d90f`): AZ1005 was READ and proved a REPOINT rather than an absence (13 more nodes to `uariz_ext_az1005`), `tools/catalog_divergence_scan.py` SHIPPED at the one non-flooding definition with turnip's 7 nodes fixed, and lavender turned out to be an UNSOURCED WINDOW rather than a relabel. 20 nodes repointed, 3 held, 6 more findings. Doc: `2026-08-05-az1005-and-divergence-followups.md`. Doc: `2026-08-05-campaign-c-reprice-and-arid-document-read.md`; reproduce with `python3 tools/campaign_c_reprice.py` (27 tests, every check mutation-tested). Kickoff 53's shape numbers ALL reproduce (116 nodes / 82 containers / 34 arms / 31 citrus). **Its adjudication test does not**: "0 of 35 carry a finding naming their REGION" is correct and is the wrong question -- these crops declare bare anchors by **SOURCE ID** in crop-scoped pilot findings, and **17 of 35 decisions carry one** (`okra_pilot_region_anchor_base_urls` alone names 3 of the 5 source ids). After the citrus re-scope C keeps **30 decisions / 85 nodes**: 1 catalog-repointable, 17 declared-anchor, 6 modeled-only, **6 honestly open** -- 5 of them one hunt. **NO DOCUMENT HUNT REMAINS.** #13's RGV guide (cached as a 403, refetched) has **ZERO rows for all six crops** and its only pea is Cowpea (*Vigna*, wrong genus); all six already declare that absence in their own `synthesis_note_seasoned` -- a THIRD adjudication vocabulary no scan checks. #17 + #24 both repoint at the Dona Ana chart, whose bars were read visually (text layer has no dates) and MATCH the data. Open work is authoring, not research |
| **D. The tail** | **18** (own: #16, #18, #23, #25-32; residue: #3-#6, #8, #14, #21) | **14 -> 26** (**91 cells**, see below) | one or two crops each; **7 of D's own 11 hunts are lemon**, and the residue is 100% citrus | **PARTLY CLOSED 2026-08-06 (PLA-114)**. Re-priced first (`campaign_d_reprice.py`, 33 tests): **the 14 was an UNDER-count**, the arc's over-count trap running in REVERSE. It counted D's own hunts only and omitted the `lemon`/`lime` residue that campaigns A and C deferred here -- which this ledger's own note column defers **seven separate times** in prose and never added to the number. Then ONE promote (`6b2dcb8e` -> `29b96b65`, commit `ae15df4`) and a follow-on credit fix (`bce8bcc7`): `uc_anr_8100` minted, `lemon.frost_tolerance_f` **28 -> 29**, **9 cells / 15 node-citations repointed**, findings F1-F5 filed, no consumer claim moved. D now measures **105 node-citations / 82 cells, all six SIBLING-PATHED decisions closed, 0 split cells**. **F1 and F5 were drafted false and rebuilt**: a scan reporting the cold threshold in "0 of 17 readable cited documents" was a proximity filter, and UF/IFAS HS1153/HS402 -- a lemon monograph the crop cites 87 times -- publishes four lemon-specific figures. Docs: `2026-08-05-campaign-d-reprice-and-citrus-document-read.md`, `2026-08-06-pla114-hs402-refutes-the-uncited-finding.md`. **REMAINING: the §7 six independent open decisions** (planting/bloom/harvest dates, claude.ai sourcing lane) + the two pears |
| | **28** | **158 -> 106** | | |

> **The arc total does NOT move with D's re-price.** D's 26 includes **12 residue decisions already
> counted under A and C**, whose citrus rows were deferred here rather than newly discovered. Adding
> them to the 158/106 totals would double-count closed work -- the same mistake in the opposite
> direction. D's own hunts contribute 14 decisions / 41 nodes; the residue contributes 12 / 82.
>
> **And the decision unit over-counts CELLS.** D's 123 node-citations are only **91 distinct
> physical cells**: one cell can carry TWO bare source ids and is then counted once per id
> (`lemon/warm_arid/plantings[0].plant_out[0]` cites bare `uariz_ext` AND bare `clemson_hgic`, so it
> appears under both hunt #30 and hunt #31, while being one cell and one authoring question). That is
> **32 cells double-counted**. 27 of the 32 take the same verdict on both arms; **five were SPLIT**,
> one arm carrying a document lead and the other not, and a split cell is not settled until both arms
> are. Campaign A corrected pairs -> decisions; this is the same correction one level down.
> Reproduce: `python3 tools/campaign_d_reprice.py --cells`.

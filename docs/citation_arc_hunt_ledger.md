...
|---|---|---|---|---|---|
| 1 | `mid_south` | `uada_ext` | 20 | **PARTIAL** | **STRAWBERRY WORKED 2026-08-03** (`76db16a`): 4 harvest nodes repointed at `uada_ext_berries`, z7 re-derived off its declared anchor, 2 findings, attribution corrected. Residue is **1 real arm** (z8 `plant_out`, unsourced) + `fig`'s 2 harvest nodes + `apple`/`elderberry` region-anchor only. **RE-PRICED: 3 of the 20 need document work** (`strawberry` 12 nodes, `fig` harvest 2, and `apple`/`elderberry` region-anchor only). 12 are claim-adjudicated with only a container root left; 4 herbs are DECLARED by their own pilot findings. Hunt 1 (2026-07-30) worked the fruit crops; the 2026-07-31 herb pass fixed **prose only**, so all 12 herb nodes still cite the bare host. `lavender`'s target is real but its nodes are **CORROBORATED, not SOLE** -- it is not in these 20. |

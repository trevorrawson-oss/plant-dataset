...
| 5 | `watering.schedule_by_stage[]` -- PER-STAGE watering for annuals (rate/amount, frequency, method, level + dual-register notes per growth stage; the "how much to water after sprouts grow" data) | **queued** (roadmap'd 2026-07-07 during timing-spine authoring) | after the structural timing spine lands across the fall/winter batch (Trevor 2026-07-07: timing-first, then watering; explicitly "in the roadmap for this work") | backfill for annuals using the EXISTING `schedule_by_stage` shape (already on ~20 trees/perennials, e.g. the blueberry anchor); per-stage, T1-sourced; its OWN focused pass -- real care content, ~2-3x the per-crop effort of the structural timing fields. Pairs with the day_range_from_sow ladder (same stage ids). | plant-app per-stage watering guidance (seedling -> established -> mature). Captures germination "keep the surface damp" + post-sprout "water deeply, less often". |
...
| 7 | STRUCTURED climate **thresholds** -- promote the per-crop heat cutoff / frost tolerance / (already-structured) germination band from PROSE into clean numeric fields (e.g. `heat_threshold_f`, `frost_tolerance_f`) | **candidate** (raised 2026-07-07 from the notifications/AI architecture discussion) | before the notifications + WeatherKit build (next week) | today the cutoffs live in prose (broccoli's ~86°F crown-failure is inside the regional `heat_pause` basis text; `germination_temp_f` exists, but heat/frost cutoffs do not as clean fields). An LLM can READ them from prose; a cheap DETERMINISTIC weather-trigger (`forecast_high > threshold -> alert`) needs them STRUCTURED. Extract into numeric fields so WeatherKit + a rule fires the common heat/frost/water alerts for free at scale, reserving the LLM for the reactive long tail. Column GS arc. Bridges the timing-spine authoring to the notification build. | plant-app notifications engine + the AI context-assembly. See memory `notifications-ai-architecture`. |

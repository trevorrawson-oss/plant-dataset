...
| 7 | STRUCTURED climate **thresholds** -- promote the per-crop heat cutoff / frost tolerance from PROSE into clean numeric fields: `heat_threshold_f` + `heat_effect`, `frost_tolerance_f` + `frost_effect` (`germination_temp_f` already structured) | **PHASE 1 DONE 2026-07-07 (contract + TDD gate + 6-crop pilot; `b15a5a0f` -> `d3a6912f`); ROLLOUT (~108 crops) HELD for Trevor's sign-off on 3 semantic decisions** | before the notifications + WeatherKit build (~week of 2026-07-14) | Column GS arc. Contract `docs/climate_thresholds_contract.md` (single-value thresholds not bands; heat=daytime-HIGH trigger, night out of scope; coherence frost<heat; heat null=N/A heat-lover, absent=TODO). Gate `tools/climate_threshold_gate.py` (TDD, `--coverage`). Pilot: broccoli/cherry-tomato/kale (heat SET, 3 effects) + okra/basil/lemon (heat N/A) + frost killed/foliage_damaged. C11: heat_effect/frost_effect ruled into register EXCLUDED_KEYS (numeric thresholds need no ruling). **Decisions flagged (contract §3):** keep the *_effect enums (rec yes); add `chilling_sensitivity_f` #7b for non-freezing chilling injury basil 50/okra 45/pepper 40 (rec yes); confirm C11 ruling. A cheap DETERMINISTIC weather-trigger fires the common alerts for free at scale, reserving the LLM for the reactive long tail. | plant-app notifications engine + AI context-assembly. Dataset already has partial scaffolding (`hard_freeze`/`FROST_PROTECT` tip tokens). See memory `notifications-ai-architecture`. |

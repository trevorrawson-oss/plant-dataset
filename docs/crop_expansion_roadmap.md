...
|---|---------|--------------------|-----------------|--------|-------|
| 1 | **Corn family** -- field/dent corn, popcorn, flint-corn | Zea mays, warm-season grass (`warm_season_grass` -> `frost_anchored`) | the sweet-corn pattern (block/row `planting_layout`, direct-sown) + the dry-bean dry-down harvest ladder (`dry_down` -> `harvest` -> `cure`), NOT sweet-corn's fresh milk-stage harvest | QUEUED | sweet-corn (certified 2026-07-10) established the block-planted warm-season-grass pattern expressly for these; NO new archetype gate (the `calendar_basis` dispatch already covers `warm_season_grass`). |

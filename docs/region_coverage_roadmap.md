...
| 9 | Mid-South region `mid_south` (AR/OK/TN/MO, **z7-8**; UAEX/NWS Little Rock T1) | dataset | **SHIPPED 2026-07-20** (canonical `af5dcee9` -> `931c1653`; 119-patch atomic promote = 111 cells + chill band + provenance + **6 new source_catalog entries**; count 128 unchanged, 119 certified). Roadmap item 9, the 4th authored region, frost-anchored, templated off mid-Atlantic. **NO new gate, NO new field.** The 3 deltas from mid-Atlantic: (1) sources NOT pre-catalogued -- registered `nws_lzk` + 5 UAEX pubs (region_harness injects them into the scratch canonical, build_region_promote emits the add-patches); (2) real intra-state chill gradient, band z7[1000,1300]/z8[900,1100] (Hope 901 warm edge clears McIntosh 900 by ~1hr); (3) blackberry the signature crop (UAEX FSA6105/UA breeding program). Fall cycle from UAEX's tighter table (tomato Jul 1-15; +3 UAEX adds sweet-corn/green-beans-bush/potato). apricot/cherry-sweet/pomegranate marginal. gate_all 119/119; footprint EXACT; register row 20; spec + kickoff #34 + dry-run; committed UNPUSHED + owes plant-app #35 (no ZIP3 fence; z7 depends on #32). NEW `tools/prose_window_sweep.py` honesty check caught a stale-VCE-window residue the sampling review missed. | surfaced by item 5's mid-South ruling (**CONDITIONAL-GO**, `docs/reviews/notes/2026-07-15/tier2_mid_south_ruling.md`): same missing-fall-tomato-window gap as mid-Atlantic, independently evidenced (2 UAEX sources); blackberry's real UA breeding-program per-cultivar chill data (Ouachita/Navaho/Apache/Kiowa/Arapaho) authored directly |
| 10 | Nevada high-desert region (z8/z9/z10, Las Vegas/Clark County anchor; UNR Extension T1) | dataset | **SHIPPED 2026-07-21** (canonical `a071f0c1` -> `9b4c478a`; 118-patch atomic promote = 111 cells + chill band + provenance + 5 new source_catalog entries; count 128 unchanged, 119 certified). The 5th authored region, high-desert archetype. **NO new gate, NO new field.** The 3 deltas: (1) warm crops SINGLE spring + summer heat_pause + NO fall replant (UNR SP-99-11; the inverse of the humid belts + Phoenix; load-bearing fix = early-Feb indoor start so Jan stays cold_pause); (2) apple `fruits_reliably` + `chill_basis` variety steering (Trevor Option A, prose-only) off the UNR SP-20-07 North Las Vegas field trial -- names the confirmed picks, flags the 700hr+ tier + Liberty; (3) garlic narrow Sep 1-Oct 15 clove window. onion/shallot intermediate_day fall-planted (A9 window-fit 0). pawpaw unsuitable (arid); citrus all cold-limited (Vegas colder than Phoenix). gate_all 119/119; footprint EXACT; register row 21; spec + plan + dry-run + kickoff #37 (ZIP3 fence 889/890/891, NO isWarm #32 dependency -- whole belt z8-10). Content fix: citrus over-attributed "colder than Phoenix" to SP-20-07 (a deciduous-fruit trial, verified) -> reattributed to elevation. | surfaced by item 5's Nevada ruling (**CONDITIONAL-GO**, `docs/reviews/notes/2026-07-15/tier2_nevada_ruling.md`): the naive calendar's flat back half is actively misleading (not just incomplete) -- misses the real Jun-Sep heat-abort period AND the real Nov frost return; apple needs real variety-chill tiering (6 of 16 canonical varieties above the confirmed 700hr trial ceiling); garlic's real fall window differs from `warm_arid`/`low_desert_az` |
| 11 | Utah "Dixie" high-desert region (St. George/Washington County z8 core; USU Extension T1) | dataset | **SHIPPED 2026-07-22** (canonical `b1045e04` -> `f7e3afe3`; 121-patch atomic promote = 111 cells + chill band + provenance + **8 new USU source sub-ids**; count 128 unchanged, 119 certified). The 6th authored region, Nevada's single-zone near-twin (`zone_span ["8"]`, St. George/Washington County). **NO new gate, NO new field** -- reuses A45/A31/A32/A3/A43/A9/chill. Single zone = the simplification vs Nevada (A45 single-zone parity, NO gradient, NO early-Feb-indoor trick since Mar 30 frost leaves Jan inactive). Sourcing lighter (`usu_ext` catalogued) but 8 per-pub sub-ids required (region_cell_audit forbids one id -> two URLs per cell). The 3 deltas: apple + pear-asian + pear-european MARGINAL (marquee flip from Nevada; Washington County Fruits page higher-elevation-only; chill_basis names the lowest-chill third Dorsett Golden/Anna/Ein Shemer); raspberry/blackberry marginal fall-bearing steer + strawberry low-elevation thriver; the honest chill-band gap ([250,450] elevation-bracketed, no USU St.-George number). onion/shallot intermediate_day fall-plant (A9 0); cherry-sour dropped to marginal on review; pawpaw unsuitable. Built SDD (cherry-tomato ref cell + 14 family shards + 2 opus content reviews, both SHIP-WITH-FIXES). gate_all 119/119; footprint EXACT; register row 22; spec/plan/notes `docs/superpowers/{specs,plans}/2026-07-22-utah-dixie-region*` + `docs/reviews/notes/2026-07-22/utah_dixie_*`. plant-app kickoff #39 (847xx fence, NO isWarm dep). COMMITTED UNPUSHED. | surfaced by item 5's Utah ruling (**CONDITIONAL-GO**, `docs/reviews/notes/2026-07-15/tier2_utah_ruling.md`): same tomato heat/frost-return gap as Nevada; apple leans marginal at St. George's elevation per county extension (would have defaulted to the canonical's `fruits_reliably` assumption, now corrected); raspberry needs fall-bearing/low-chill cultivar steering, confirmed via this dataset's own pre-existing `warm_arid` raspberry text (same USU source) |

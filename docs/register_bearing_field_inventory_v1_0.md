...
**Lavender woody-ornamental addendum (2026-06-22, ruled by Trevor at the lavender anchor; closes the register_completeness HALT on lavender Steps 1-3, the FIRST `perennial_woody_ornamental`).** Two new prose fields surfaced. (1) `varieties.recommended[].hardiness_note` (per-variety hardiness label, e.g. "Hardy, reliably perennial in roughly zones 5 to 9.") -- **ruled USER-FACING-CATEGORICAL** (§4, no suffix, left bare, dash-gated): a terse single-form per-variety attribute consistent with the `varieties.recommended[].use`/`note`/`delta.*` categorical rulings already on the same array. Gate rule: `ruled_categorical` matches `k == "hardiness_note"` under `varieties.recommended[]`. (2) `container_notes.drainage.saucer_practice` (e.g. "Empty the saucer after watering; lavender must never sit in standing water.") -- **RE-RULED USER-FACING-CATEGORICAL, SUPERSEDING its earlier §2.1 CP listing**: a single plain drainage instruction read identically by both registers. Lavender is the FIRST crop to populate the field (the CP default was never exercised); it authored a bare string, Trevor confirmed CATEGORICAL. Gate rule: `ruled_categorical` matches `k == "saucer_practice"` under `container_notes`. NOTE: the two new woody-ornamental window-note fields (`pruning_window.note`, `dormancy_window.note`) were authored as proper CP pairs (`note_seasoned`/`note_beginner`) and need no §4 ruling. (Plant-project working copy + project knowledge owe the same refresh.)

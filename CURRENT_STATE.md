...
- **Checklist Step 0 finalization (claude.ai authors -> Claude Code promotes -> Trevor refreshes PK):** ratify the region shell-build rule above + the admission gate into the next `gold_standard_arc_checklist` version. Proposal staged: Downloads `proposal_step0_shell_admission_gate.md`. Folds pending amendment (2) shell-pass-first + WORRY 3 (gates as admission HALT).

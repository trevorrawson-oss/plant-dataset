...
## Open items owed
- **Register-completeness gate (Claude Code, building next):** a structural gate that walks the union of prose fields across ALL crops and asserts each has a ruling in `register_bearing_field_inventory_v1_0.md` -- any unruled prose field is a finding. Durable fix for the bolting class (inventory was tomato-anchored and missed `bolting`). Folds into Step 11 + the shell pass.

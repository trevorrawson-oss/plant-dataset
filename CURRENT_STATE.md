...
- **Governing checklist: v1.4.1.** Dataset is authoritative for what is true; flag doc lag.
...
- **Checklist v1.5 finalization (Claude Code authored the step spec + gate; claude.ai integrates -> Claude Code promotes -> Trevor refreshes PK):** add **Step 3.5 -- Region shell build** (Claude Code lane, builds all 10 regions to the reference-GS-crop shape; north=zones promote+verify+hoist; absorbs the northern_tier satellite doc), reconcile Step 4 scope, two-callsite gate model (WORRY 3), reference-GS-crop pointer. Authored content + claude.ai prompt staged: Downloads `checklist_v1_5_shell_build_amendment.md`. Trevor's call: this is a v1.5 (minor) bump.

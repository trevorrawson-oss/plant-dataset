...
- **Succession-shape rule (spec-B):** the succession RULE lives once in region-constant `plantings[]`; `resolved_by_zone[z]` holds only date-strings + `calendar[]` (the app reads the rule layer and IGNORES `resolved_by_zone`). Do NOT enumerate succession arms in `resolved_by_zone`.
...
- **Optional forward finding:** machine-readable fall heat-floor clamp field, dataset-wide, if the app resolver must reproduce the cold-zone Jul-1 clamp exactly.

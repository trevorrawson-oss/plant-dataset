...
## Canonical pointer
- **Current SHA:** `815efe623033a7ab5c164911970e58d953cf1f9f4fae1435b06aa70606c15aff` (register-conversion completion addendum -- 10,801 prose fields converted: 9,264 SP `_seasoned` renames + 1,537 CP `_seasoned`+null `_beginner`; the nested + array-name prose the 2.8 walk missed).
- **Predecessor chain:** `8a1d8a50` (bolting addendum) <- `ed495666` (Pass 2 warm-cell) <- `20f9fc2b` (Pass 1b succession hoist) <- `327a2d5c` (Pass 1a calendars) <- `582dbbad` (northern_tier reconciliation).
...
## Open items owed
- **Register-conversion completion addendum -- DONE (`815efe62`).** claude.ai ruled the gate's candidates; Claude Code applied: Bucket A (SP `synthesis_note`/`design_note`/`source_quote`/`zone_coverage_note` 9,247) -> `_seasoned` only; Bucket B (growth_stages_annual/year_one inner prose 750) -> CP; Bucket C (storage/watering/yield/rotation/container/tasks/tip/safe_sowing_note + count_note/heat_pause.basis/microclimate_note) per ruling; C-CATEGORICAL (rotation.good_after, thinning.when, fruit_size/color) left bare. Structural-verifier proof + all counts matched. 2 nuances handed back to claude.ai (`for_chat` handoff): (1) `safe_sowing_note` already had AUTHORED beginner copy -> kept, not null-scaffolded; (2) thousands of NULL-valued occurrences (`source_quote` 2,165 null, `zone_coverage_note` 460, `saucer_practice`/`shape_requirements` 121 each) left bare per the string-only counts -- claude.ai to rule leave-bare vs `_seasoned:null` for key-uniformity.
- **Register-completeness gate -- BUILT (draft):** `plant-dataset/tools/register_completeness_gate.py` (roster-completeness inversion: any prose-shaped field with no ruling HALTS; stop-and-ask, no auto-fix). **Refinement still owed:** load the positive CP/SP roster so it auto-separates "ruled-but-unconverted" from "genuinely unruled"; after the addendum + refinement, a clean re-run -> **0 unruled** is the acceptance signal. **Home:** per-crop shell pass admission gate + Step 11.
